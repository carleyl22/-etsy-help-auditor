"""                                                                           
  ...
  <style>                                                                       
      ...
      .issue-critical { background-color: #f8d7da; padding: 10px; border-radius:
   5px; margin: 5px 0; }                                                        
      .issue-warning { background-color: #fff3cd; padding: 10px; border-radius: 
  5px; margin: 5px 0; }                                                         
      .issue-suggestion { background-color: #d1ecf1; padding: 10px;             
              ...
              with st.spinner("Connecting..."):                                 
                  try:                                                          
                      ...
                      if not client.test_connection():                          
                          st.error("Failed to connect to Zendesk. Check your    
  credentials.")                                                                
                          return                                                
          ...
          if st.session_state.audit_history:                                    
              ...
              for i, audit in                                                   
  enumerate(reversed(st.session_state.audit_history[-5:])):                     
  ...
  def render_score_card(report):                                                
      ...
      with col1:                                                                
          ...
          st.markdown(f"<div                                                    
  class='{score_class}'>{report.overall_score}/100</div>",                      
  unsafe_allow_html=True)                                                       
  ...
  "🔵"}.get(issue.severity, "⚪")                                               
   ...
   ""}                                                                          
              {f"<br><em>Recommendation:</em> {issue.recommendation}" if        
  issue.recommendation else ""}                                                 
          </div>                                                                
          """, unsafe_allow_html=True)                                          
  ...
  def main():                                                                   
      ...
      st.markdown("Audit help articles for content quality, technical accuracy, 
  and Etsy style guidelines.")                                                  
                                                                                
      connect_services()                                                        
                                                                                
      # Main content area                                                       
      tab1, tab2, tab3 = st.tabs(["🔍 Single Article Audit", "📚 Batch Audit",  
  "📊 Search Articles"])                                                        
   ...
   or 123456789",                                                               
          ...
          with col2:                                                            
              st.write("")  # Spacing                                           
              st.write("")                                                      
              audit_button = st.button("🔍 Audit Article", type="primary",      
              ...
              if report:                                                        
                  ...
                  with findings_tabs[0]:                                        
                      ...
                      with col2:                                                
                          ...
                          st.write(f"Web instructions: {'✅' if                 
  report.has_web_instructions else '❌'}")                                      
                          st.write(f"App instructions: {'✅' if                 
  report.has_app_instructions else '❌'}")                                      
                                                                                
                      if report.member_services_flag:                           
                          st.error(f"⚠️ **Member Services Flag:**               
  {report.flag_reason}")                                                        
                  ...
                  with findings_tabs[6]:                                        
                      st.write(f"**UI Elements Found:**                         
  {report.ui_elements_total}")                                                  
  ...
  3456790\n123456791"                                                           
          ...
          if st.button("🔍 Audit All", type="primary", disabled=not             
  ...
  a.strip()]                                                                    
                  ...
                  for i, article in enumerate(articles):                        
                      status.text(f"Auditing {i+1}/{len(articles)}:             
  {article[:50]}...")                                                           
                  ...
                  if results:                                                   
                      ...
                      df = pd.DataFrame([                                       
                          {                                                     
                              ...
                              "MS Flag": "Yes" if r.member_services_flag else   
  "No"                                                                          
                          }                                                     
                          for r in results                                      
                      ])                                                        
   ...
   in results])                                                                 
      ...
      with tab3:                                                                
          ...
          search_query = st.text_input("Search query", placeholder="refund      
  policy")                                                                      
          ...
          if st.button("🔍 Search", disabled=not st.session_state.connected):   
              if search_query:                                                  
                  with st.spinner("Searching..."):                              
                      try:                                                      
                          results =                                             
  st.session_state.zendesk_client.search_articles(search_query)                 
                          if results:                                           
                              for article in results[:10]:                      
                                  with st.expander(article.title):              
                                      st.write(f"**ID:** {article.id}")         
                                      st.write(f"**URL:** {article.html_url}")  
                                      if st.button(f"Audit",                    
  key=f"audit_{article.id}"):                                                   
                                          report =                              
  audit_article(str(article.id))                                                
                          else:                                                 
